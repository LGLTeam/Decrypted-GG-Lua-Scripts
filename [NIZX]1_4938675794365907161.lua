...
if os.date("%Y%m%d") > "20200411" then
...
function name(A0_11)
  if _NIZX0_ ~= A0_11 then
    _NIZX0_ = A0_11
    if #gg.getRangesList(_NIZX0_) == 0 then
      os.exit()
    else
      _NIZX3_ = gg.getRangesList(_NIZX0_)[1].start
      _NIZX4_ = gg.getRangesList(_NIZX0_)[#gg.getRangesList(_NIZX0_)]["end"]
    end
  end
end

function hex2tbl(A0_12)
  local L1_13
  L1_13 = {}
  A0_12:gsub("%S%S", function(A0_14)
    local L1_15
    L1_15 = L1_13
    L1_15 = #L1_15
    L1_15 = L1_15 + 1
...
function original(A0_16)
  if #hex2tbl(A0_16) == 0 then
    return
  end
  if #hex2tbl(A0_16) > _NIZX1_ then
  end
  for _NZX_8_ = 1, _NIZX1_ do
    if _NZX_8_ ~= 1 then
    end
    if hex2tbl(A0_16)[_NZX_8_] == "??" or hex2tbl(A0_16)[_NZX_8_] == "**" then
    end
  end
  gg.searchNumber((("" .. ";") .. "0~~0" .. "r") .. "::" .. _NIZX1_, gg.TYPE_BYTE, false, gg.SIGN_EQUAL, _NIZX3_, _NIZX4_)
  if #hex2tbl(A0_16) > _NIZX1_ then
    for _NZX_8_ = _NIZX1_ + 1, #hex2tbl(A0_16) do
      if hex2tbl(A0_16)[_NZX_8_] == "??" or hex2tbl(A0_16)[_NZX_8_] == "**" then
  ...
  while not (gg.getResultCount() <= 0) and not (#gg.getResults(8) < 0 + _NIZX1_) do
    ...
    while #gg.getResults(8) > 0 do
      if #hex2tbl(A0_16) > _NIZX1_ then
        for _NZX_17_ = _NIZX1_ + 1, #hex2tbl(A0_16) do
        end
        for _NZX_17_ = _NIZX1_ + 1, #hex2tbl(A0_16) do
          if hex2tbl(A0_16)[_NZX_17_] ~= 256 and gg.getValues({})[_NZX_17_ - _NIZX1_].value ~= hex2tbl(A0_16)[_NZX_17_] then
            break
          end
        end
      end
      if false then
        _NIZX5_[0 + 1] = gg.getResults(8)[1 + 0].address - 1
      else
...
function replaced(A0_17)
  _NIZX0_ = _NIZX0_ + 1
  if _NIZX2_ ~= nil then
    for _NZX_6_, _NZX_7_ in ipairs((hex2tbl(A0_17))) do
  ...
  for _NZX_8_, _NZX_9_ in ipairs(_NIZX3_) do
    for _NZX_13_, _NZX_14_ in ipairs((hex2tbl(A0_17))) do
...
function vel()
  ...
  if n == nil then
    ...
    Hex = string[L0_5("af b8 bb b6 aa bd")]("%X", n[1])
    if string[L0_5("b5 ae b7")](Hex) == 1 then
      Hex = "0" .. Hex
    end
