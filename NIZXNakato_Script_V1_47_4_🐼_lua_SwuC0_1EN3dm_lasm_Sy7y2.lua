...
function original(A0_5)
  ...
  if #hex2tbl(A0_5) > _NIZX1_ then
  end
  for _NZX_8_ = 1, _NIZX1_ do
    if _NZX_8_ ~= 1 then
    end
    if hex2tbl(A0_5)[_NZX_8_] == "??" or hex2tbl(A0_5)[_NZX_8_] == "**" then
    end
  end
  gg.searchNumber((("" .. ";") .. "0~~0" .. "r") .. "::" .. _NIZX1_, gg.TYPE_BYTE, false, gg.SIGN_EQUAL, _NIZX2_, _NIZX3_)
  if #hex2tbl(A0_5) > _NIZX1_ then
    for _NZX_8_ = _NIZX1_ + 1, #hex2tbl(A0_5) do
      if hex2tbl(A0_5)[_NZX_8_] == "??" or hex2tbl(A0_5)[_NZX_8_] == "**" then
      elseif ("0x" .. 256) + 0 > 127 then
      end
      hex2tbl(A0_5)[_NZX_8_] = ("0x" .. 256) + 0 - 256
    end
  end
  _NIZX4_ = {}
  while not (gg.getResultCount() <= 0) and not (#gg.getResults(8) < 0 + _NIZX1_) do
    for _NZX_14_, _NZX_15_ in ipairs((gg.getResults(8))) do
      _NZX_15_.address = _NZX_15_.address + myoffset
    end
    gg.loadResults((gg.getResults(8)))
    while #gg.getResults(8) > 0 do
      if #hex2tbl(A0_5) > _NIZX1_ then
        for _NZX_17_ = _NIZX1_ + 1, #hex2tbl(A0_5) do
        end
        for _NZX_17_ = _NIZX1_ + 1, #hex2tbl(A0_5) do
          if hex2tbl(A0_5)[_NZX_17_] ~= 256 and gg.getValues({
            [_NZX_17_ - _NIZX1_] = {
              address = gg.getResults(8)[1 + 0].address - 1 + _NZX_17_,
              flags = gg.TYPE_BYTE,
              value = 0
            }
          })[_NZX_17_ - _NIZX1_].value ~= hex2tbl(A0_5)[_NZX_17_] then
            break
          end
        end
      end
      if false then
        ...
        gg.removeResults({
          [_NZX_17_] = gg.getResults(8)[_NZX_17_ + 0]
        })
      end
    end
  end
...
function replaced(A0_6)
  _NIZX0_ = _NIZX0_ + 1
  if _NIZX2_ ~= nil then
    for _NZX_6_, _NZX_7_ in ipairs((hex2tbl(A0_6))) do
      if _NZX_7_ ~= "??" and _NZX_7_ ~= "*" and _NZX_7_ == hex2tbl(_NIZX2_)[_NZX_6_] then
        hex2tbl(A0_6)[_NZX_6_] = "*"
      end
    end
    _NIZX2_ = nil
  end
  for _NZX_8_, _NZX_9_ in ipairs(_NIZX3_) do
    for _NZX_13_, _NZX_14_ in ipairs((hex2tbl(A0_6))) do
